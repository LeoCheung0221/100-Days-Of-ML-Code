<p align="center"><a href="day-54.md">中文</a> &nbsp;&nbsp;·&nbsp;&nbsp; <b>English</b></p>

# Day 54 · Drop volume

[Phase I · Models](../../README.en.md) · runs

What you learn today: five lags 0.000081; with volume 0.000121; rise −0.000040; volume helped=false

## Plain-language account

Day 54's numbers come from script stdout, not hand-filled values. rise=mse5−mse6=−0.000040. 五 lag alone 更好. 加 volume hurt test MSE，不能记成增益. 

The panel is days/data/panel.csv, name AAA, simple returns from adjusted close. Train is the first seventy-five percent in time order unless this script changes the cut. Hold-out rows score MSE only; coefficients are not re-fit there. Same-bar high, low, close are not features; same-day market is not a result.

Separate train from test: parameters on train, MSE on hold-out mean squared error. five lags 0.000081; with volume 0.000121; rise −0.000040; volume helped=false

Return MSE is not price-level SSE from days 45–46. Claims serve this print only.

## Core

```text
test MSE five lags only = 0.000081
test MSE five lags and volume = 0.000121
MSE rise when volume removed = -0.000040
volume helped on the test stretch = false
```

## Further out

Cross-check stdout against the page: key names, signs, and six decimals should match the terminal. five lags 0.000081; with volume 0.000121; rise −0.000040; volume helped=false

Keep forbidden rules beside MSE in the lab notebook. If the next lesson changes the cut or target, open a new log row instead of overwriting today's numbers.

## What the run showed

```bash
python days/54-drop-volume/drop_volume.py
```

The script should print stdout lines matching the core block. The implementation is [`drop_volume.py`](../../days/54-drop-volume/drop_volume.py).

Hand in the printed numbers and rule lines for day 54. Keep the script path for reruns.
