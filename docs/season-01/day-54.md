...
第 54 天的数字来自脚本 stdout，不是手算补值。rise=mse5−mse6=−0.000040。五 lag alone 更好。加 volume hurt test MSE，不能记成增益。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。五 lag test MSE 0.000081；加 volume 0.000121；MSE rise −0.000040；volume helped=false
...
volume 列进入设计矩阵后 test MSE 从 0.000081 升到 0.000121。MSE rise when volume removed 打印 −0.000040，表示去掉 volume 后五 lag 模型更好。volume helped=false 应读成「加 volume hurt test MSE」，不是「volume 缺失」。
...
对照实验命名要清楚：five lags only 是基线，five lags and volume 是加列。rise 行负号方向别反读；helped=false 对应 hurt test。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。rise=mse5−mse6=−0.000040。五 lag alone 更好。加 volume hurt test MSE，不能记成增益。
...
今天交出去的是第 54 天打印表上的数与规则句。请保留脚本路径便于复跑。
