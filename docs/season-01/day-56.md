...
第 56 天把 lag-5 任务写成 stdout 合同：task 行说明用 AAA adj_close 上五个滞后收益预测当日简单收益；target 行重申标签是同日简单收益；forbidden 行禁止同行 high/low/close 作特征。train rows = 54 与 test rows = 19 必须与默认百分之七十五时间切分一致。test MSE = 0.000081 仍是第 51 天 frozen 直线在 hold-out 上的标尺。
