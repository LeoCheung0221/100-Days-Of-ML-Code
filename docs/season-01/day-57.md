...
第 57 天的数字来自脚本 stdout，不是手算补值。三列 OHLC 都印 FORBIDDEN。feature build rejects same-row OHLC=true。合法特征仍五 lag。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。high/low/close 均 FORBIDDEN same-bar；允许五 lag；test MSE=0.000081
...
high、low、close 三列均标 FORBIDDEN same-bar，feature build rejects same-row OHLC=true。允许特征仍只有 five lagged returns。test MSE 0.000081 说明在拒绝 same-bar 价格后，五 lag 线仍给出同一 hold-out 分数。
...
FORBIDDEN 行对 high/low/close 三列重复，是强调 same-bar 而非笔误。allowed features 行是白名单，只剩 five lagged returns。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。三列 OHLC 都印 FORBIDDEN。feature build rejects same-row OHLC=true。合法特征仍五 lag。
...
今天交出去的是第 57 天打印表上的数与规则句。请保留脚本路径便于复跑。
