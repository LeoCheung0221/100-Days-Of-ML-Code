...
第 58 天的数字来自脚本 stdout，不是手算补值。按日期切：早于 2024-02-28 训练。四十行测试。0.000782 远大于 0.000081，切分规则改变分数。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。切点 2024-02-28，train=33，test=40，test MSE=0.000782，差于 75% 切分 0.000081
...
日历切点 2024-02-28：train=33、test=40。test MSE 0.000782 明显高于 seventy-five percent 时间切分的 0.000081，必须并列写两句，不能只说「模型变差」而不说「切分变了」。四十行测试集与十九行测试集的 MSE 不可纵向拼接成一条曲线。
...
对比 0.000782 与 0.000081 时，要写清前者是 calendar cut、后者是 seventy-five percent。train=33 样本较少，估计方差也会放大 test MSE。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。按日期切：早于 2024-02-28 训练。四十行测试。0.000782 远大于 0.000081，切分规则改变分数。
...
今天交出去的是第 58 天打印表上的数与规则句。请保留脚本路径便于复跑。
