...
第 60 天的数字来自脚本 stdout，不是手算补值。improvement=baseline−line=0.000020。index 13 是 test 上 |y−ŷ| 最小日，误差 0.000214。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。baseline 0.000101，line 0.000081，improvement 0.000020；index 13 error 0.000214
...
MSE improvement over baseline=0.000101−0.000081=0.000020。smallest line error day index on test=13，该日 |y−ŷ|=0.000214，是单日绝对误差，不是 MSE 平方根。报告 improvement 时不要混用 index 行的 error 与整体 MSE。
...
index=13 只对应 test 段行号，不是日历日期。0.000214 是 absolute error，与 MSE 0.000081 不同量纲，不可直接比较大小论优劣。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。improvement=baseline−line=0.000020。index 13 是 test 上 |y−ŷ| 最小日，误差 0.000214。
...
今天交出去的是第 60 天打印表上的数与规则句。请保留脚本路径便于复跑。
