...
Tree test MSE 0.000174 loses to baseline 0.000101; improvement −0.000072.

The stump loses to both the line and predicting zero on this stretch.

Price-level tree wins do not transfer to this return setup.
...
Day 61 tree test MSE 0.000174 gives negative improvement over baseline −0.000072.
...
Day 61 prints 3 contract lines:

- `baseline test MSE = 0.000101`: match the terminal verbatim—keys, spacing, signs, six decimals.
- `tree test MSE = 0.000174`: match the terminal verbatim—keys, spacing, signs, six decimals.
- `MSE improvement over baseline = -0.000072`: match the terminal verbatim—keys, spacing, signs, six decimals.
...
Day 62 shifts to MAE and top-decile slices.

Cite negative improvement when claiming trees beat baseline here.

Tree loses to baseline with improvement −0.000072. Do not import price-tree SSE. Day 62 shifts to MAE slices.
...
The script should print stdout matching the core block. Implementation: [`tree_vs_baseline.py`](../../days/61-tree-vs-baseline/tree_vs_baseline.py).
