...
第 61 天的数字来自脚本 stdout，不是手算补值。负 improvement：树比猜零更差。0.000174 与第 52 天 test 树 MSE 一致。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。baseline 0.000101，tree 0.000174，improvement −0.000072；树未 beat 零基准
...
tree test MSE=0.000174，baseline=0.000101，improvement=−0.000072。负号表示树比猜零更差。这与第 52 天 test 树 MSE 一致，应交叉引用同一台阶规则，而不是引用价格水平树的 SSE。
...
improvement 负值应加粗在日志结论里：树未 beat 零基准。不要把 tree 与 line 的 improvement 混在一行。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。负 improvement：树比猜零更差。0.000174 与第 52 天 test 树 MSE 一致。
...
今天交出去的是第 61 天打印表上的数与规则句。请保留脚本路径便于复跑。
