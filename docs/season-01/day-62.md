...
第 62 天的数字来自脚本 stdout，不是手算补值。置信=|ŷ|。最高 tenth 仅 2 行。top MAE>rest，大 |ŷ| 不等于小误差。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。test=19，top=2，top MAE 0.007193，rest 0.006955；top tenth 未更准
...
置信度取 |ŷ|，top tenth count=2（十九行 test 的 ceil(10%)）。top tenth mean absolute error=0.007193 大于 rest=0.006955，故 top tenth 并不更准。不要用 |ŷ| 当「可交易信号」而不报 MAE 对照。
...
top count=2 来自 ceil(19×0.1)。比较 top 与 rest MAE 时保留六位小数。结论句：top tenth is NOT more accurate。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。置信=|ŷ|。最高 tenth 仅 2 行。top MAE>rest，大 |ŷ| 不等于小误差。
...
今天交出去的是第 62 天打印表上的数与规则句。请保留脚本路径便于复跑。
