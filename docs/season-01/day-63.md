...
第 63 天的数字来自脚本 stdout，不是手算补值。test |y−ŷ| 按 yyyy-mm 聚合。三月 2 天，四月 17 天；读均值必带 days。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。2024-03 mean 0.007770 days=2；2024-04 mean 0.006887 days=17
...
按月聚合 test 绝对误差：2024-03 仅 2 天，mean=0.007770；2024-04 有 17 天，mean=0.006887。短月均值波动大，读 March 时必须带 days=2，避免把 0.007770 当成与 17 天同权重。
...
打印顺序按 month 排序。April 十七天主导直觉，但 March 两天仍占一行，不能删。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。test |y−ŷ| 按 yyyy-mm 聚合。三月 2 天，四月 17 天；读均值必带 days。
...
今天交出去的是第 63 天打印表上的数与规则句。请保留脚本路径便于复跑。
