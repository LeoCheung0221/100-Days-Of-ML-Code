...
今天的学习要点：2024-03 mean 0.007770 days=2；2024-04 mean 0.006887 days=17

## 费曼法讲解

把 test 段按日历月聚合 MAE：2024-03 只有 2 个交易日，均值 0.007770；2024-04 有 17 天，均值 0.006887。

小样本月波动大，不能把 2 天均值当成稳定季节因子。

全 test MAE 0.006980（第 72 天）介于两者之间，由天数加权。
...
第 63 天 stdout 核心块共 2 行。下面逐行说明读法纪律（不是改写成口语数字）：

- `month 2024-03 mean abs error = 0.007770 days = 2`：键名、等号两侧空格、负号与小数位须与终端一致。中文解释可以写长，但这行英文与数字是 diff 基准；不要把 false 写成中文「否」、不要把 FORBIDDEN 行删掉、不要把 bill −18 写成 18。若该行含 test MSE，默认指 hold-out 平均平方误差；若含 FORBIDDEN，表示该列不得进入合法特征。批改时对此行做逐字 diff，而不是只看摘要段。
- `month 2024-04 mean abs error = 0.006887 days = 17`：键名、等号两侧空格、负号与小数位须与终端一致。中文解释可以写长，但这行英文与数字是 diff 基准；不要把 false 写成中文「否」、不要把 FORBIDDEN 行删掉、不要把 bill −18 写成 18。若该行含 test MSE，默认指 hold-out 平均平方误差；若含 FORBIDDEN，表示该列不得进入合法特征。批改时对此行做逐字 diff，而不是只看摘要段。
...
第 64 天问：去掉最好月后整体 MAE 如何变。

写月表时务必带 days= 列，避免把「月均值低」误解为「月内天天准」。

## 与前后课的关系

按月 MAE：2024-03 仅 2 天，均值 0.007770；2024-04 有 17 天，均值 0.006887。小月方差大，不要把 3 月写成「模型在春季更差」；样本只有两天。第 64 天会做「去掉最好月」ablation；本日先建立月表。全 test MAE 0.006980 在第 72 天出现，可与月表对照加权关系。写月报时务必带 days= 列，防止把月均值当成日频保证。
...
脚本应打印与核心块一致的 stdout 行。实现是 [`monthly_residuals.py`](../../days/63-monthly-residuals/monthly_residuals.py).
...
第 63 天补记：lag-5 合同锚点包括 line test MSE 0.000081、volume helped=false（第 54 天）、total bill line=−18.0000（第 75 天）。改切分或 name 会改分数，但未重跑脚本时不得手改上述字面量。