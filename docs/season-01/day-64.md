...
第 64 天的数字来自脚本 stdout，不是手算补值。drop 最小 mean 月 2024-04。去掉后只剩 March 两行，均值升到 0.007770。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。最小误差月 2024-04 mean 0.006887；全月 0.006980；去掉后 0.007770（剩 March）
...
最小 mean abs 月=2024-04（0.006887）。全 test mean=0.006980；去掉 2024-04 后只剩 March，mean=0.007770 上升。这说明「最好月」被去掉后，整体误差被小样本月主导，删月实验必须报告剩余 days。
...
without that month 行 mean=0.007770 等于 March-only，与 2024-03 行 mean 对齐检查。drop 月 2024-04 是误差最小月，不是样本最多月。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。drop 最小 mean 月 2024-04。去掉后只剩 March 两行，均值升到 0.007770。
...
今天交出去的是第 64 天打印表上的数与规则句。请保留脚本路径便于复跑。
