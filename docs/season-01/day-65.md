...
第 65 天的数字来自脚本 stdout，不是手算补值。同一五 lag 线与 75% 切分。BBB 更差但同量级。换 name 必须重印 MSE。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。AAA MSE 0.000081；BBB 0.000105；勿自动搬结论
...
AAA test MSE=0.000081，BBB=0.000105。同一公式、不同 name，分数不同；不要把 AAA 结论自动搬到 BBB。BBB 更差但仍为六位小数同量级，应分行记录而非合并平均。
...
两行 name= 应独立评分卡。BBB 0.000105 仍可用同一 pipeline 复跑验证。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。同一五 lag 线与 75% 切分。BBB 更差但同量级。换 name 必须重印 MSE。
...
今天交出去的是第 65 天打印表上的数与规则句。请保留脚本路径便于复跑。
