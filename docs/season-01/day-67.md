...
第 67 天的数字来自脚本 stdout，不是手算补值。FORBIDDEN 与 not a result 句同脚本。此处 0.000094>0.000081，同日 market hurt。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。lags only 0.000081；加 same-day market 0.000094；FORBIDDEN；更低 MSE 也不是结果
...
lags only test MSE=0.000081；加 same-day market 列得 0.000094。脚本印 FORBIDDEN 与 a lower MSE with same-day market is not a result。此处 0.000094>0.000081，同日 market 仍 hurt，但即使更低也不算合法结果。
...
两行 MSE 都要打印，即使 FORBIDDEN。not a result 句保护未来更低 MSE 的误读。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。FORBIDDEN 与 not a result 句同脚本。此处 0.000094>0.000081，同日 market hurt。
...
今天交出去的是第 67 天打印表上的数与规则句。请保留脚本路径便于复跑。
