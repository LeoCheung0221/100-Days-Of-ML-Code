...
第 68 天的数字来自脚本 stdout，不是手算补值。pipeline 封装第 65 天逻辑。entry 行标记入口。换 name 只改参数。
...
读数时先把训练与测试分开：参数只在训练段估计，MSE 只在 hold-out 行上算平均平方误差。pipeline(name)：AAA 0.000081，BBB 0.000105
...
entry=pipeline(name) 封装同一五 lag 线。AAA=0.000081，BBB=0.000105，与第 65 天数字一致。单一入口便于审计：换 name 只改参数，公式与切分不变。
...
pipeline 函数名与 entry 行是审计入口。两 name 分数与 day 65 应一致，可作为回归测试。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。pipeline 封装第 65 天逻辑。entry 行标记入口。换 name 只改参数。
...
今天交出去的是第 68 天打印表上的数与规则句。请保留脚本路径便于复跑。
