...
第 73 天的数字来自脚本 stdout，不是手算补值。方向只看 sign(y) 与 sign(ŷ)，不看误差幅度。3/19 约十六 percent 失手，与 MAE 或 MSE 不是同一分数。
...
读数时先把训练与测试分开：五 lag 直线只在训练段估计系数，本课所有计数与均值只在 hold-out 十九行上算。方向错 3 天；测试共 19 天。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。方向只看 sign(y) 与 sign(ŷ)，不看误差幅度。3/19 约十六 percent 失手，与 MAE 或 MSE 不是同一分数。
...
今天交出去的是第 73 天打印表上的数与规则句。请保留脚本路径便于复跑。
