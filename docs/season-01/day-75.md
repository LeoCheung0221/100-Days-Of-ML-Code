...
第 75 天的数字来自脚本 stdout，不是手算补值。账单在测试段逐日累加：方向错扣 1，jump 扣 3，同一日两项都满足就扣 4。3×(−1)+5×(−3)=−18，与打印一致。
...
读数时先把训练与测试分开：五 lag 直线只在训练段估计系数，本课所有计数与均值只在 hold-out 十九行上算。账单 total bill line=−18.0000；方向错 3；jump 日 5。规则：方向错 −1，jump 日 −3，可同日叠加。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。账单在测试段逐日累加：方向错扣 1，jump 扣 3，同一日两项都满足就扣 4。3×(−1)+5×(−3)=−18，与打印一致。
...
今天交出去的是第 75 天打印表上的数与规则句。请保留脚本路径便于复跑。
