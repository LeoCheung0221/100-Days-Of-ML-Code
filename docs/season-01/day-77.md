...
第 77 天的数字来自脚本 stdout，不是手算补值。发言日定义 |ŷ|≥0.01。frozen 直线在测试段上没有任何 |ŷ| 达到 0.01，故 speaking=0，条件 MAE 不定义。
...
读数时先把训练与测试分开：五 lag 直线只在训练段估计系数，本课所有计数与均值只在 hold-out 十九行上算。threshold=0.0100；days speaking=0；speaking 时 MAE=not defined。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。发言日定义 |ŷ|≥0.01。frozen 直线在测试段上没有任何 |ŷ| 达到 0.01，故 speaking=0，条件 MAE 不定义。
...
今天交出去的是第 77 天打印表上的数与规则句。请保留脚本路径便于复跑。
