...
阈值 0.0010 时 seventeen 日发言，speaking MAE=0.007663，接近全 test MAE 0.006980 但略高。

与第 77 天对照：同一 |ŷ| 分布，门槛决定覆盖率与条件 MAE 是否可定义。

不要把 speaking 天数当成交易次数——仍 no order。
...
第 78 天 stdout 核心块共 3 行。下面逐行说明读法纪律（不是改写成口语数字）：

- `threshold = 0.0010`：键名、等号两侧空格、负号与小数位须与终端一致。中文解释可以写长，但这行英文与数字是 diff 基准；不要把 false 写成中文「否」、不要把 FORBIDDEN 行删掉、不要把 bill −18 写成 18。若该行含 test MSE，默认指 hold-out 平均平方误差；若含 FORBIDDEN，表示该列不得进入合法特征。批改时对此行做逐字 diff，而不是只看摘要段。
- `days speaking = 17`：键名、等号两侧空格、负号与小数位须与终端一致。中文解释可以写长，但这行英文与数字是 diff 基准；不要把 false 写成中文「否」、不要把 FORBIDDEN 行删掉、不要把 bill −18 写成 18。若该行含 test MSE，默认指 hold-out 平均平方误差；若含 FORBIDDEN，表示该列不得进入合法特征。批改时对此行做逐字 diff，而不是只看摘要段。
- `mean abs error when speaking = 0.007663`：键名、等号两侧空格、负号与小数位须与终端一致。中文解释可以写长，但这行英文与数字是 diff 基准；不要把 false 写成中文「否」、不要把 FORBIDDEN 行删掉、不要把 bill −18 写成 18。若该行含 test MSE，默认指 hold-out 平均平方误差；若含 FORBIDDEN，表示该列不得进入合法特征。批改时对此行做逐字 diff，而不是只看摘要段。
...
第 79 天比较 line 与 tree 的 total bill。

## 与前后课的关系

threshold=0.0010，speaking=17，speaking MAE=0.007663，接近 all-test 0.006980 略高。与第 77 天对照覆盖 vs 静音。第 79 天比 line 与 tree 账单。条件 MAE 只在 speaking 子集定义；分母是 17 不是 19。报告时写清 threshold 与 speaking count。
...
第 78 天补记：lag-5 合同锚点包括 line test MSE 0.000081、volume helped=false（第 54 天）、total bill line=−18.0000（第 75 天）。改切分或 name 会改分数，但未重跑脚本时不得手改上述字面量。