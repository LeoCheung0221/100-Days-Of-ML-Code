...
第 79 天的数字来自脚本 stdout，不是手算补值。树桩用训练段 best stump 预测测试段，账单规则与直线相同。树方向错更多或 jump 叠加更重，得 −27；直线 −18 更少负，故 line 赢。
...
读数时先把训练与测试分开：五 lag 直线只在训练段估计系数，本课所有计数与均值只在 hold-out 十九行上算。同第 75 天规则：line bill=−18，tree bill=−27；lower bill wins=line（少负为佳）。
...
把当天 stdout 与正文交叉核对：核心块中的英文键名、符号和小数位应与终端一致。树桩用训练段 best stump 预测测试段，账单规则与直线相同。树方向错更多或 jump 叠加更重，得 −27；直线 −18 更少负，故 line 赢。
...
今天交出去的是第 79 天打印表上的数与规则句。请保留脚本路径便于复跑。
