<p align="center"><b>中文</b> &nbsp;&nbsp;·&nbsp;&nbsp; <a href="day-90.en.md">English</a></p>

# 第 90 天 · 三个失手日

[第一阶段 · 模型](README.md) · 可运行

今天的学习要点：fail rank 1–3 日期与 reason：两 jump horizontal miss、一 direction wrong
...
本课按 **水平绝对误差** \(|r-\hat y|\) 对 hold-out 十九行降序，打印前三名日期与 **reason** 标签。Rank1 **2024-04-03**、Rank2 **2024-04-18** 为 **jump day horizontal miss**（|r| 达 p75 且水平误差大）；Rank3 **2024-04-22** 为 **direction wrong**（符号错）。

Reason 规则与第 71 天分类一致：先看 jump（p75），再看 direction，其余 quiet size miss——但只打印 top3。Jump horizontal miss **不要求** direction wrong；第 82 天高波动 jump MAE 大与此一致。Direction wrong 日可以 |r| 不大但仍入 top3，因为排序纯看 |error|。

本课是 **ex post 失败分析**，不是 training loss 剪枝。勿把 rank 日当作「应剔除样本」除非 prespec 删点规则（第 94 天是删整月，不同 estimand）。Almgren & Chriss（2000）提醒 tail 日执行成本更高；第 92 天仍假设 close fill。

工程师应能在 panel 上按日期复算 |r−ŷ| 并验证排序；reason 列依赖 jump/direction 掩码，与第 84 天表交叉验证。
...
**锚点。** 首行或关键行以 `fail rank 1 date = 2024-04-03` 标识本课；其余行按 `day_90()` 顺序打印。
...
**第 90 天专属检查。** 跑完脚本后先复制 stdout 到笔记，再写中文解释；顺序错误会导致 diff 失败。

**键名锚点。** `fail rank 1 date = 2024-04-03` 应出现在 ```text``` 块；slide 标题可用中文，但脚注必须含该行英文。
...
**阅读检查（第 90 天）。** 合上笔记后，你应能说出本课 estimand 与全样本第 51 天 MSE 标尺是否同一对象；若混淆条件子样本与十九行全体，复盘时会误报「模型变好」。

**第 90 天与批改。** 助教只比对 ```text``` 与终端；中文段落写错键名仍会通过 diff，但会误导未来的你——务必把英文键复制进 slide 脚注。
...
**第 90 天复盘。** 对照终端逐行复制 stdout；中文只解释定义与 estimand，不改写键名、不等号空格或小数位数。
...
亦可：`python3 -c "from days.run_day import main; main(90)"`。

实现：[`three_failures.py`](../../days/90-three-failures/three_failures.py)。交付：stdout 与 ```text``` 块逐字一致。
