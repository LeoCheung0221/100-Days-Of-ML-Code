<p align="center"><b>中文</b> &nbsp;&nbsp;·&nbsp;&nbsp; <a href="day-92.en.md">English</a></p>

# 第 92 天 · 成交假设三行

[第一阶段 · 模型](README.md) · 可运行

今天的学习要点：fill 三行：close 信号、零滑点无下单、只算误差与 bill
...
三行 **成交假设 manifest** 锁定 scoring 与实盘差距：`fill row 1` 信号按当日 **adj_close** 形成；`fill row 2` **脚本不发单、滑点为零**；`fill row 3` **不算 PnL，只算误差与 bill**。

这是 **research protocol 边界**：第 69 天起已暗示；92 写成 diff 行供 risk 阅读。Close fill 意味着无 intraday path；零滑点意味着无 bid-ask；无 PnL 意味着 bill 是 **计数成本表** 而非美元损益。Almgren & Chriss（2000）说明真实执行会改变 alpha——本季 deliberately 不建模。

勿从三行推出「可以实盘按 close 成交」；它们只说明 **脚本假设**。若第 93 天 one-tick slippage 改变 direction 计数，才需升级 fill 模型；当前面板 rank changed=false。

Quant 组会应把 92 三行放在 backtest slide 脚注；没有这三行，direction/bill 讨论缺少执行语境。
...
**锚点。** 首行或关键行以 `fill row 1 = signal at close uses adj_close that day` 标识本课；其余行按 `day_92()` 顺序打印。
...
**第 92 天专属检查。** 跑完脚本后先复制 stdout 到笔记，再写中文解释；顺序错误会导致 diff 失败。

**键名锚点。** `fill row 1 = signal at close uses adj_close that day` 应出现在 ```text``` 块；slide 标题可用中文，但脚注必须含该行英文。
...
**阅读检查（第 92 天）。** 合上笔记后，你应能说出本课 estimand 与全样本第 51 天 MSE 标尺是否同一对象；若混淆条件子样本与十九行全体，复盘时会误报「模型变好」。

**第 92 天与批改。** 助教只比对 ```text``` 与终端；中文段落写错键名仍会通过 diff，但会误导未来的你——务必把英文键复制进 slide 脚注。
...
**第 92 天复盘。** 对照终端逐行复制 stdout；中文只解释定义与 estimand，不改写键名、不等号空格或小数位数。
...
亦可：`python3 -c "from days.run_day import main; main(92)"`。

实现：[`fill_three_lines.py`](../../days/92-fill-three-lines/fill_three_lines.py)。交付：stdout 与 ```text``` 块逐字一致。
