<p align="center"><b>中文</b> &nbsp;&nbsp;·&nbsp;&nbsp; <a href="day-93.en.md">English</a></p>

# 第 93 天 · 最小滑点

[第一阶段 · 模型](README.md) · 可运行

今天的学习要点：slippage 0.0001 不改变 direction wrong 计数 3；rank changed = false

## 费曼法讲解

本课在预测上施加 **one-tick slippage 0.0001**（对 ŷ 做减号方向调整，见 `day_93()`），比较 direction wrong **前后** 计数。Before **3**、after **3**、`rank changed = false` 说明 **在本面板该 tick 下符号诊断不变**。

Slippage 只动 **预测阈值附近** 的符号，不是成本模型完整版。Tick 0.0001 相对日收益尺度约 1bp 量级；对多数日 sign 不变，但对 near-zero 预测日可能翻转——本 hold-out 未翻转。若 rank changed 为 true，memo 须讨论 **稳健性**；此处 false 仍应报告，避免读者以为未测 slippage。

与 92 三行联读：92 声明脚本默认零滑点；93 是 **敏感性探针**，不是新 baseline。Execution quant 应扫 tick grid；本课只给一个点。

Direction wrong 定义仍 sign(y) vs sign(ŷ_adj)；bill 未在本课重算——若 sign 变了 bill 可能变，本脚本只数 direction。
...
**锚点。** 首行或关键行以 `slippage one tick = 0.0001` 标识本课；其余行按 `day_93()` 顺序打印。
...
**第 93 天专属检查。** 跑完脚本后先复制 stdout 到笔记，再写中文解释；顺序错误会导致 diff 失败。


**键名锚点。** `slippage one tick = 0.0001` 应出现在 ```text``` 块；slide 标题可用中文，但脚注必须含该行英文。
...
**阅读检查（第 93 天）。** 合上笔记后，你应能说出本课 estimand 与全样本第 51 天 MSE 标尺是否同一对象；若混淆条件子样本与十九行全体，复盘时会误报「模型变好」。


**第 93 天与批改。** 助教只比对 ```text``` 与终端；中文段落写错键名仍会通过 diff，但会误导未来的你——务必把英文键复制进 slide 脚注。
...
**第 93 天复盘。** 对照终端逐行复制 stdout；中文只解释定义与 estimand，不改写键名、不等号空格或小数位数。
...
亦可：`python3 -c "from days.run_day import main; main(93)"`。

实现：[`slippage_tick.py`](../../days/93-slippage-tick/slippage_tick.py)。交付：stdout 与 ```text``` 块逐字一致。
