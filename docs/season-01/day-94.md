<p align="center"><b>中文</b> &nbsp;&nbsp;·&nbsp;&nbsp; <a href="day-94.en.md">English</a></p>

# 第 94 天 · 去掉最优月份

[第一阶段 · 模型](README.md) · 可运行

今天的学习要点：drop 2024-03 后 line MSE 0.000080、tree 0.000170；model kept = line
...
本课做 **stress 删样本**：在 hold-out 上按月份聚合 |error| 均值，去掉均值最大的月 **2024-03**，再在剩余日上比较 line vs stump MSE。Line **0.000080**、tree **0.000170**、`model kept after drop = line`。

删月改变 **estimand**：不再是标准十九行 hold-out，而是 **去掉一个 calendar month 后的子集**。这是 deliberate 展示「删最优 stress 月后结论是否仍成立」——不是 prespec 生产规则。若 silently drop 最好月再报 MSE，属于 optimistic bias；此处 stdout 明示 dropped month。

0.000080 略低于全样本 0.000081，说明 2024-03 相对难；但 line 仍 beat tree。第 85 天在高波动周选模；94 是 **时间轴删点**，几何不同。

工程师须核对 `month dropped` 与 panel 日期对齐；改切分后 best month 可能变。
...
**锚点。** 首行或关键行以 `month dropped = 2024-03` 标识本课；其余行按 `day_94()` 顺序打印。
...
**第 94 天专属检查。** 跑完脚本后先复制 stdout 到笔记，再写中文解释；顺序错误会导致 diff 失败。

**键名锚点。** `month dropped = 2024-03` 应出现在 ```text``` 块；slide 标题可用中文，但脚注必须含该行英文。
...
**阅读检查（第 94 天）。** 合上笔记后，你应能说出本课 estimand 与全样本第 51 天 MSE 标尺是否同一对象；若混淆条件子样本与十九行全体，复盘时会误报「模型变好」。

**第 94 天与批改。** 助教只比对 ```text``` 与终端；中文段落写错键名仍会通过 diff，但会误导未来的你——务必把英文键复制进 slide 脚注。
...
**第 94 天复盘。** 对照终端逐行复制 stdout；中文只解释定义与 estimand，不改写键名、不等号空格或小数位数。
...
亦可：`python3 -c "from days.run_day import main; main(94)"`。

实现：[`drop_best_month.py`](../../days/94-drop-best-month/drop_best_month.py)。交付：stdout 与 ```text``` 块逐字一致。
