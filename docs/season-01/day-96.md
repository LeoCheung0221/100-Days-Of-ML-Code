<p align="center"><b>中文</b> &nbsp;&nbsp;·&nbsp;&nbsp; <a href="day-96.en.md">English</a></p>

# 第 96 天 · 一页说明

[第一阶段 · 模型](README.md) · 可运行

今天的学习要点：one page 四标题：task、split、baseline、errors 词汇

## 费曼法讲解

四行 **one page title** 是 slide 骨架：`task` 五 lag 预测 AAA 收益；`split` 75% 时间序 train；`baseline` hold-out 零预测；`errors` quiet/jump/direction 与 bill 词汇。不含数字——数字分散在 89、99、100 等课；96 只锁 **叙事结构**。

好的一页纸应先定义 estimand 再报数字；96 强迫你把 **task/split/baseline/errors** 四格填满才进入 MSE。第 100 天 narrative 是口述版 superset；96 是 **标题级** 压缩。

Teaching intent：从 day 86–87 manifest 到 96 四标题，学员应能串成 onboarding deck。若四标题与 87 forbidden 冲突，deck 不合格。

工程师可将四行复制到 README 顶部；键名须与 stdout 一致以便未来 diff。
...
**锚点。** 首行或关键行以 `one page title task = predict AAA return from five lags` 标识本课；其余行按 `day_96()` 顺序打印。
...
**第 96 天专属检查。** 跑完脚本后先复制 stdout 到笔记，再写中文解释；顺序错误会导致 diff 失败。


**键名锚点。** `one page title task = predict AAA return from five lags` 应出现在 ```text``` 块；slide 标题可用中文，但脚注必须含该行英文。
...
**阅读检查（第 96 天）。** 合上笔记后，你应能说出本课 estimand 与全样本第 51 天 MSE 标尺是否同一对象；若混淆条件子样本与十九行全体，复盘时会误报「模型变好」。


**第 96 天与批改。** 助教只比对 ```text``` 与终端；中文段落写错键名仍会通过 diff，但会误导未来的你——务必把英文键复制进 slide 脚注。
...
**第 96 天复盘。** 对照终端逐行复制 stdout；中文只解释定义与 estimand，不改写键名、不等号空格或小数位数。
...
亦可：`python3 -c "from days.run_day import main; main(96)"`。

实现：[`one_page_four.py`](../../days/96-one-page-four/one_page_four.py)。交付：stdout 与 ```text``` 块逐字一致。
