<p align="center"><b>中文</b> &nbsp;&nbsp;·&nbsp;&nbsp; <a href="day-99.en.md">English</a></p>

# 第 99 天 · 全流程再运行

[第一阶段 · 模型](README.md) · 可运行

今天的学习要点：full run MSE 0.000081、direction wrong 3、matches day 51 = true

## 费曼法讲解

**Full rerun 验收** 四行：`full run line test MSE = 0.000081`；`direction wrong days = 3`；`matches day 51 test MSE = true`；`manifest matches day 70 ten lines = true`。把 **水平、方向、manifest** 一次对齐，防止 refactor 后 silent drift。

这是 season-01 的 **integration test 文档化**：任何改 `run_day` 或 panel 路径后应先过 99。Direction 3 与第 71 天合同一致；MSE true 要求六位字符串与 day 51 完全相同。

Manifest true 连接第 70、86–87 十行摘要；若 70 改一行，99 变 false 直到同步。CI 理想状态应跑 99 作为 gate。

工程师发布前 checklist：跑 99；截图 stdout；与 ```text``` diff；再 merge。
...
**锚点。** 首行或关键行以 `full run line test MSE = 0.000081` 标识本课；其余行按 `day_99()` 顺序打印。
...
**第 99 天专属检查。** 跑完脚本后先复制 stdout 到笔记，再写中文解释；顺序错误会导致 diff 失败。


**键名锚点。** `full run line test MSE = 0.000081` 应出现在 ```text``` 块；slide 标题可用中文，但脚注必须含该行英文。
...
**阅读检查（第 99 天）。** 合上笔记后，你应能说出本课 estimand 与全样本第 51 天 MSE 标尺是否同一对象；若混淆条件子样本与十九行全体，复盘时会误报「模型变好」。


**第 99 天与批改。** 助教只比对 ```text``` 与终端；中文段落写错键名仍会通过 diff，但会误导未来的你——务必把英文键复制进 slide 脚注。
...
**第 99 天复盘。** 对照终端逐行复制 stdout；中文只解释定义与 estimand，不改写键名、不等号空格或小数位数。
...
亦可：`python3 -c "from days.run_day import main; main(99)"`。

实现：[`full_rerun.py`](../../days/99-full-rerun/full_rerun.py)。交付：stdout 与 ```text``` 块逐字一致。
